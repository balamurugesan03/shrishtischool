...
const INIT = { date: new Date().toISOString().split('T')[0], type: 'Credit', accountHead: '', description: '', amount: 0, paymentMode: 'Cash', voucherNumber: '' };
...
export default function Ledger() {
  ...
  const fetchLedger = useCallback(async () => {
    setLoading(true);
    try {
      const res = await ledgerAPI.getAll({ page, limit, type: filterType, startDate: startDate || undefined, endDate: endDate || undefined });
      setData(res.data);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await ledgerAPI.create({ ...formValues, date: formValues.date ? new Date(formValues.date) : new Date() });
      enqueueSnackbar('Ledger entry created', { variant: 'success' });
      setFormOpen(false); fetchLedger();
      setFormValues(INIT);
    } catch (err) {
  ...
  const balance = (totals.Credit || 0) - (totals.Debit || 0);

  const columns = [
    { key: 'date', label: 'Date', render: v => new Date(v).toLocaleDateString() },
    { key: 'voucherNumber', label: 'Voucher #' },
    { key: 'accountHead', label: 'Account Head' },
    { key: 'description', label: 'Description' },
    { key: 'type', label: 'Type', render: v => <Chip label={v} size="small" color={v === 'Credit' ? 'success' : 'error'} variant="outlined" sx={{ height: 20, fontSize: '11px', fontWeight: 600 }} /> },
    { key: 'amount', label: 'Amount', render: (v, row) => <Typography variant="body2" fontWeight={500} color={row.type === 'Credit' ? 'success.main' : 'error.main'}>₹{v?.toLocaleString()}</Typography> },
    { key: 'paymentMode', label: 'Mode' }
  ];

  return (
    <>
      <PageHeader title="Ledger" subtitle="General accounting ledger"
        action={<Button variant="contained" startIcon={<IconPlus size={16} />} onClick={() => setFormOpen(true)}>Add Entry</Button>} />

      <Grid container spacing={2} mb={2}>
        {[
          { label: 'Total Credit', value: `₹${(totals.Credit || 0).toLocaleString()}`, color: 'success.main' },
          { label: 'Total Debit', value: `₹${(totals.Debit || 0).toLocaleString()}`, color: 'error.main' },
          { label: 'Net Balance', value: `₹${Math.abs(balance).toLocaleString()}`, color: balance >= 0 ? 'primary.main' : 'error.main' },
        ].map(card => (
      ...
      <Box sx={{ display: 'flex', gap: 1.5, mb: 2, flexWrap: 'wrap' }}>
        <TextField select size="small" label="Type" value={filterType}
          ...
          {['Credit','Debit'].map(t => <MenuItem key={t} value={t}>{t}</MenuItem>)}
        </TextField>
      ...
      <Dialog open={formOpen} onClose={() => setFormOpen(false)} maxWidth="xs" fullWidth>
        <form onSubmit={handleSubmit}>
          <DialogTitle fontWeight={700}>Add Ledger Entry</DialogTitle>
          <DialogContent>
            <Stack spacing={2} pt={1}>
              ...
              <TextField label="Account Head" size="small" fullWidth required
                value={formValues.accountHead} onChange={set('accountHead')} placeholder="e.g. Fee Income, Salary Expense" />
              ...
              <TextField label="Voucher Number" size="small" fullWidth value={formValues.voucherNumber} onChange={set('voucherNumber')} />
            </Stack>
          </DialogContent>
