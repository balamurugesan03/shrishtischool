...
export default function StaffAttendanceHistory() {
  ...
  }, [page, limit, filterDate, filterType, filterStaff]);
  ...
  const columns = [
    {
      ...
      render: (val) => (
        <Box>
          ...
          <Typography variant="caption" color="primary">{val?.staffId}</Typography>
        </Box>
      )
    },
    {
      ...
      label: 'Department',
      render: (_, row) => (
        <Typography variant="body2">{row?.staff?.department}</Typography>
      )
    },
    {
      key: 'staff_role',
      label: 'Role',
      render: (_, row) => (
        <Typography variant="body2">{row?.staff?.role}</Typography>
      )
    },
    {
      key: 'type',
      ...
      render: (val) => (
        <Chip
          label={val === 'IN' ? 'CHECKED IN' : 'CHECKED OUT'}
  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle="View all staff IN / OUT attendance records"
        action={
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
        {/* Staff dropdown */}
        <TextField
          select label="Staff Member" size="small" value={filterStaff}
        ...
        >
          ...
          <MenuItem value="IN">Checked In (IN)</MenuItem>
          <MenuItem value="OUT">Checked Out (OUT)</MenuItem>
        </TextField>
      </Box>
