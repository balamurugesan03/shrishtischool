...
import {
  ...
  IconChevronRight, IconSchool, IconLogout,
  IconBook, IconWallet, IconCoin, IconChartBar,
  IconBackpack, IconSettings,
  ...
  return (
    <AppShell.Navbar className={classes.navbar} p={0}>
      ...
      <Box className={classes.navInner}>
        ...
        <Box className={classes.brand}>
          <Box className={classes.brandIcon}>
            <IconSchool size={22} color="#fff" stroke={2} />
          </Box>
          {!collapsed && (
            <>
              <Box style={{ flex: 1, overflow: 'hidden' }}>
                <Text className={classes.brandName} truncate>Bright Future</Text>
                <Text className={classes.brandSub}>School Management</Text>
              </Box>
              ...
              {NAV_SECTIONS.map((section, idx) => (
                <Box key={section.title}>
                  {idx > 0 && (
                    <Box
                      className={
                        collapsed ? classes.collapsedDivider : classes.sectionDivider
                      }
                    />
                  )}
                  {!collapsed && (
      ...
      <Box className={classes.userSection}>
        {collapsed ? (
          <Tooltip label="Admin User" position="right" withArrow offset={14} color="dark">
            <Box className={classes.userAvatarOnly}>
              <Avatar size={36} radius="xl" className={classes.avatar}>AD</Avatar>
            </Box>
          </Tooltip>
        ) : (
          <Box className={classes.userCard}>
            <Avatar size={36} radius="xl" className={classes.avatar}>AD</Avatar>
            <Box style={{ flex: 1, overflow: 'hidden' }}>
              <Text className={classes.userName} truncate>Admin User</Text>
              <Text className={classes.userRole}>Administrator</Text>
            </Box>
            <Tooltip label="Logout" withArrow>
              <UnstyledButton className={classes.logoutBtn}>
